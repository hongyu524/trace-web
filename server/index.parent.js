...
// OpenAI API key validation removed - OpenAI calls must be made from Vercel serverless functions only
// Railway backend no longer uses OpenAI
console.log('[SERVER] OpenAI removed from Railway backend - all OpenAI calls must go through Vercel');
...
function findFfmpegPath() {
        ...
        for (const ffmpegPath of lines) {
          if (ffmpegPath.endsWith('.exe')) {
            ...
            const versionResult = spawnSync(ffmpegPath, ['-version'], {
              stdio: 'ignore',
              windowsHide: true,
              shell: false, // Never use shell for FFmpeg execution
            });
  ...
  // On Linux, resolve full path using 'which' to avoid shell issues
  if (process.platform !== 'win32') {
    try {
      const whichResult = spawnSync('which', ['ffmpeg'], {
        stdio: 'pipe',
        shell: false,
      });
      if (whichResult.status === 0 && whichResult.stdout) {
        ...
        // Verify it works
        const versionResult = spawnSync(fullPath, ['-version'], {
          stdio: 'ignore',
          shell: false,
        });
        if (versionResult.status === 0) {
          console.log(`[FFMPEG_DETECTION] Found in PATH: ${fullPath}`);
          return fullPath; // Return full path on Linux
        }
      }
    } catch (e) {
      console.warn(`[FFMPEG_DETECTION] 'which ffmpeg' failed: ${e.message}`);
      // Continue to fallback
    }
  }
  
  // Fallback: Try direct 'ffmpeg' command (Windows only - Linux should have found via 'which' above)
  // On Linux, we must never use shell mode to avoid syntax errors with special characters
  if (process.platform === 'win32') {
    try {
      const result = spawnSync('ffmpeg', ['-version'], {
        stdio: 'ignore',
        windowsHide: true,
        shell: true, // Windows PATH resolution requires shell
      });
      if (result.status === 0) {
    ...
    // On Linux, if 'which' failed, we cannot safely use 'ffmpeg' without full path
    // This should not happen if FFmpeg is properly installed
    console.warn(`[FFMPEG_DETECTION] FFmpeg not found via 'which' on Linux - may not be installed`);
  }
...
app.post('/api/plan-memory', async (req, res) => {
  try {
    ...
    // OpenAI removed from Railway - always use deterministic fallback
    // All OpenAI calls must be made from Vercel serverless functions only
    console.log('[PLANNER] OpenAI removed from Railway - using deterministic plan');
    const plan = createDeterministicPlan(photos, promptText);
    plan.usedPlanner = 'fallback';
    console.log("[PLANNER][FALLBACK_REASON]", "openai-removed-from-railway");
...
function runFfmpeg(ffmpegPath, args, context = 'ffmpeg', timeoutMs = 120000) {
  ...
  // Never use shell mode - it causes syntax errors with special characters (parentheses, etc.) in filter_complex
  // spawn() handles array arguments correctly without shell interpretation
  const useShell = false;
  
  // PHASE D: Log FFmpeg command start (arguments only, no shell interpretation)
  console.log(`[FFMPEG][${context}] Starting FFmpeg command (timeout: ${timeoutMs/1000}s)`);
  console.log(`[FFMPEG][${context}] Executable path: ${ffmpegPath}`);
  console.log(`[FFMPEG][${context}] Arguments (${args.length}):`, JSON.stringify(args));
  // Also log as string for readability (but note: this is for display only, not executed)
  const cmdString = `${ffmpegPath} ${args.join(' ')}`;
  console.log(`[FFMPEG][${context}] Command (display only): ${cmdString}`);
...
app.post('/api/create-memory-OLD', async (req, res) => {
  ...
  try {
    ...
    // OpenAI removed from Railway - vision analysis must be called from Vercel serverless functions
    // if (!process.env.OPENAI_API_KEY) {
    //   throw new Error('OpenAI API key required for vision analysis. Please set OPENAI_API_KEY environment variable.');
    // }
    
    // TODO: Call Vercel serverless function for vision analysis instead
    throw new Error('Vision analysis requires OpenAI and must be called from Vercel. Railway backend no longer supports OpenAI.');
    
    /* COMMENTED OUT: Unreachable code after throw statement above
    try {
      // Analyze all images
    ...
    */
    ...
    // OpenAI removed from Railway - sequence planning must be called from Vercel serverless functions
    // TODO: Call Vercel serverless function for sequence planning instead
    throw new Error('Sequence planning requires OpenAI and must be called from Vercel. Railway backend no longer supports OpenAI.');
...
// Sequence planning endpoint - returns optimal image ordering using OpenAI
app.post('/api/sequence', async (req, res) => {
  try {
    ...
    const { images, context, aspectRatio, frameRate } = req.body;
    ...
    for (let i = 0; i < images.length; i++) {
      const img = images[i];
      if (!img.id) {
        return res.status(400).json({ error: `Image at index ${i} must have an 'id' field` });
      }
      if (!img.url && !img.base64) {
        return res.status(400).json({ error: `Image at index ${i} must have either 'url' or 'base64' field` });
      }
    }
    ...
    if (!apiKey) {
      console.error('[SEQUENCE] OPENAI_API_KEY environment variable is not set');
    ...
    console.log(`[SEQUENCE] Using model: ${modelName} for ${images.length} images`);

    // Initialize OpenAI client (don't log the key)
    const openai = new OpenAI({ apiKey });

    // Build prompt for sequence ordering
    const imageList = images.map((img, idx) => 
      `Image ${idx}: id="${img.id}"${img.url ? ` (URL provided)` : ` (base64 provided)`}`
    ).join('\n');

    const systemPrompt = `You are a professional video editor creating a cinematic memory video sequence plan.

Your task is to order ALL provided images into the best story sequence (no omissions).

OUTPUT FORMAT (JSON only, no markdown):
{
  "order": [0, 1, 2, ...],
  "beats": ["opening", "build", "climax", "resolution", ...],
  "rationale": "brief explanation of the sequence"
}

HARD CONSTRAINTS:
1) order must be an array of indices (0 to n-1) representing the new order
2) All indices must appear exactly once (order.length == images.length)
3) beats array is optional but should match the order length if provided
4) Each beat should be one of: "opening", "build", "climax", "resolution", "transition"
5) Avoid near-duplicate adjacency when possible
6) Create emotional arc from beginning to end

Return ONLY valid JSON, no markdown code blocks.`;

    const userPrompt = `Context: ${context || '(none provided)'}
Aspect Ratio: ${aspectRatio || '16:9'}
Frame Rate: ${frameRate || 24} fps

Analyze these ${images.length} images and determine the optimal cinematic ordering:
${imageList}

Return the order array (indices), optional beats array, and optional rationale.`;
    ...
    try {
      ...
      try {
        ...
        console.error('[SEQUENCE] Response text:', responseText.substring(0, 500));
        return res.status(500).json({ 
      ...
      if (!Array.isArray(parsed.order)) {
        return res.status(500).json({ error: 'OpenAI response missing or invalid "order" array' });
      }

      // Validate order contains all indices
      const expectedIndices = new Set(images.map((_, idx) => idx));
      const orderSet = new Set(parsed.order);
      if (orderSet.size !== expectedIndices.size || ![...orderSet].every(idx => expectedIndices.has(idx))) {
        console.warn('[SEQUENCE] Order validation failed, using fallback');
        // Fallback: return original order
        parsed.order = images.map((_, idx) => idx);
      }
      ...
      console.log(`[SEQUENCE] Successfully generated order for ${images.length} images`);
      ...
      console.error('[SEQUENCE] OpenAI API error:', openaiError.message);
    ...
    console.error('[SEQUENCE] Unexpected error:', error);
    return res.status(500).json({ 
...
app.get('/api/health', async (req, res) => {
  ...
  // OpenAI removed from Railway - no longer checking for OpenAI API key
  ...
  if (req.query.verbose) {
    ...
    console.log(`[HEALTH] OpenAI: removed from Railway (must use Vercel)`);
